...
onMounted(async () => {
  ...
  try {
    mediaStream = await navigator.mediaDevices.getUserMedia({ video: true,audio: true }); // mediaStream을 열어 영상과 목소리 넣기
        ...
        <BoothBack class="booth-camera-box">
          <div ref="captureArea" :style="{ backgroundImage: `url(${bgImage})` }" class="photo-zone" >
            <div v-show="isvideoOn">
                  <video ref="videoElement" autoplay  style="width: 30%; height: fit-content;"></video>
                </div> 
          </div>
