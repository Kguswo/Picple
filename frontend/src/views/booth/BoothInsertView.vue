...
const extractSizeFromFilename = (filename) => {
    ...
    if (match) {
        console.log(
            `Extracted size from filename: ${filename}, width: ${match[1]}, height: ${match[2]}`
        );
        return {
...
const imagePositions = computed(() => {
    if (
        !selectedImage.value ||
        !templateImageSize.value.width ||
        !templateImageSize.value.height
    )
        return [];

    const template =
        templateInfo[selectedImage.value.split("/").pop().split(".")[0]];
    if (!template) return [];

    return template.layout.map((item) =>
        calculateImagePosition(templateImageSize.value, item)
    );
});
...
const onDrop = (event, index) => {
    ...
    if (draggedImage.value) {
        console.log(
            `이미지 드롭됨: ${draggedImage.value.src} at index ${index}`
        );
        ...
        images.value.find(
            (img) => img.src === draggedImage.value.src
        ).visible = false;
...
const onDragStartFromTemplate = (event, index) => {
    if (droppedImages.value[index]) {
        console.log(
            `템플릿 이미지 드래그 시작: ${droppedImages.value[index].src}`
        );
...
const updateTemplateDimensions = () => {
    ...
    console.log(
        `Template dimensions updated: ${JSON.stringify(
            templateImageSize.value
        )}`
    );
};
        ...
        <div class="booth-content">
            <div class="booth-content-main">
                <BoothBack class="booth-temp-box">
                    <div class="selected-image-area" ref="templateRef">
                        <img
                        ...
                        >
                            <img
                                ...
                                draggable="true"
                                @dragstart="
                                    onDragStartFromTemplate($event, index)
                                "
                            />
                    ...
                    <div class="photo-list">
                        ...
                        >
                            <img
                                ...
                                draggable="true"
                                @dragstart="
                                    (event) => onDragStart(event, image, index)
                                "
            ...
            <div class="booth-footer">
                <button @click="onSave" class="save-button">저장</button>
            </div>
