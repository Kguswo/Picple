  ...
  try {
    ...
    session.value.on("streamCreated", (event) => {
      ...
      subscriber.on('streamManagerStarted', () => {
        console.log(`[2-3] 스트림 매니저 시작됨: ${subscriber.stream.streamId}`);
      });
...
const applySegmentation = (streamRef) => {
  ...
  console.log('[세그멘테이션] Selfie Segmentation 설정 완료');

  const onResults = (results) => {
    console.log('[세그멘테이션] 프레임 처리 결과 수신');
    const canvasElement = document.createElement("canvas");
    ...
    console.log('[세그멘테이션] 새 비디오 트랙으로 교체 완료');
  };
  ...
  camera.start();
  console.log('[세그멘테이션] 카메라 시작 및 프레임 처리 시작');
};
