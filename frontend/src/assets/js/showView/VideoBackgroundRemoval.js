export default class VideoBackgroundRemoval {
  ...
  async createProcessedStream(inputVideoElement) {
      const canvas = document.createElement('canvas');
      canvas.width = inputVideoElement.videoWidth;
      canvas.height = inputVideoElement.videoHeight;
      this.initCanvas(canvas);

      const processFrame = async () => {
          if (inputVideoElement.videoWidth > 0 && inputVideoElement.videoHeight > 0) {
              await this.selfieSegmentation.send({image: inputVideoElement});
          }
