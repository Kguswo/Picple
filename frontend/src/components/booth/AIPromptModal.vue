<template>
    <div id="app">
      <div class="modal-overlay" @click.self="closeModal">
        <div class="modal-content">
          ...
          <!-- 예시 내용 -->
          <div v-show="showExamples" class="examples">
            <p>"은하와 별들이 빼곡히 몽환적으로 수놓여 있는 광활한 우주"</p>
            <p>"일몰 때의 현대적인 옥상 테라스, 부드러운 조명과 편안한 좌석이 있는 도시 전경을 그려줘."</p>
          </div>
          
          <textarea v-model="prompt" placeholder=" 여기에 프롬프트를 입력해 주세요!"></textarea>
  ...
  <script setup>
  import { ref } from 'vue';
  import { backgroundAIGenerationApi } from '@/api/boardApi'; // 수정된 API 호출 import
  
  const props = defineProps({
    userId: {
      type: String,
      required: true, // userId 필수
    },
  });
  ...
  const generatePrompt = async () => {
    try {
      ...
      const response = await backgroundAIGenerationApi(prompt.value);
