...
<template>
  <div class="form-input-conatiner">
    <div class="input-container"
      ...
      @click="focusInput">
      <input :type="inputParams.type" v-model="inputParams.value" ref="inputField" class="form-input"
        :class="{ 'has-content': inputParams.value, 'background-color-disabled': isSend === true }"
        :maxlength="maxLength" :disabled="isSend === true" autocomplete="off" />
      <label class="form-label">{{ inputParams.label }}</label>

      <div class="form-input-etc">
        <FormIconCancelComp v-if="inputParams.value" class="form-icon-cancel" @click="cancelInput" />
        <FormIconVisibilityComp v-if="inputParams.label.includes('비밀번호') && inputParams.value"
          class="form-icon-visibility" :type="inputParams.type" @toggle-visibility="toggleVisibility" />
        <slot></slot>
      </div>
    </div>
